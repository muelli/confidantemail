...
# Automatic client to let users set SMTP notification addresses
# Hard coded settings, times are in seconds
check_for_acks_interval = 3600
#DEBUG#poll_interval = 120
poll_interval = 30
key_post_interval = 86400
notify_file = 'c:\\projects\\keymail\\notify.txt'
server_name = 'test.confidantmail.org'
passphrase = "test"
...
re_email = re.compile("^\S+@\S+\.\S+$",re.IGNORECASE)

notify_server_instructions = \
"Confidant Mail SMTP email notification\n\
\n\
Commands:\n\
notify <email address> - set an email address for SMTP notification\n\
cancel - disable SMTP notification for your account\n\
\n\
Examples:\n\
notify sampleuser@confidantmail.org\n\
cancel\n"

class autoclient_notify:
	...
	def main(self,argv):
		cmdline = argv[1:]
		if len(cmdline) == 0:
			print "Usage: python autoclient_notify.py -homedir /path"
			sys.exit(1)
...
#					sys.exit(2)
	...
	def check_poll(self):
		...
		if (self.check_send_enabled == True) and \
		   (nowtime - self.last_poll_time > self.poll_interval):
			...
			# Ignoring the bypass token here, so I can run on two servers
			# without worrying about out-of-sync incoming tokens
			self.to_agent_queue.put( [ 'CHECK_SEND',self.lastCheckSendDate,True ] )
	...
	def update_notify_file(self,keyid,notify_address):
		lock_file = notify_file + '.lock'
		notify_file_new = notify_file + '.new'
		notify_file_old1 = notify_file + '.old1'
		notify_file_old2 = notify_file + '.old2'
		notify_file_old3 = notify_file + '.old3'
		keyidL = keyid.lower()
		lfh = open(lock_file,'w')
		ifh = open(notify_file,'r')
		ofh = open(notify_file_new,'w')
		if notify_address != None:
			notify_line_out = keyidL + ',email,' + notify_address
		for line in ifh:
			line = line.rstrip('\r\n\t ')
			lineL = line.lower()
			if lineL[0:40] == keyidL:
				if notify_address != None:
					ofh.write(notify_line_out + '\n')					
					notify_line_out = None
			else:
				ofh.write(line + '\n')
		if notify_address != None and notify_line_out != None:
			ofh.write(notify_line_out + '\n')					
		ifh.close()
		ofh.close()
		if os.path.exists(notify_file_old3):
			os.unlink(notify_file_old3)
		if os.path.exists(notify_file_old2):
			os.rename(notify_file_old2,notify_file_old3)
		if os.path.exists(notify_file_old1):
			os.rename(notify_file_old1,notify_file_old2)
		os.rename(notify_file,notify_file_old1)
		os.rename(notify_file_new,notify_file)
		lfh.close()
		os.unlink(lock_file)
...
#			pass
		...
		subject = "Notify server response"
		attachments = [ ]
		...
		for line in body_text.split('\n'):
			...
			if lineL[0:6] == 'cancel':
				cancel_notify = True
			elif lineL[0:7] == 'notify ':
				notify_address = line[7:]
				if re_email.match(notify_address) == None:
					notify_address = None

		body_text = ''

		if cancel_notify == True:
			self.update_notify_file(fromKeyid,None)
			body_text = "Email notification canceled on server " + server_name
		elif notify_address != None:
			self.update_notify_file(fromKeyid,notify_address)
			body_text = "Email notification address set to " + notify_address + " on server " + server_name
		else:	
			subject = "Notify server instructions"
			body_text = notify_server_instructions
