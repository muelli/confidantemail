...
# Mike - suppress unhandled errors due to store etc. timing out
# The local node update does happen, but then the exception gets re-thrown
# and not handled, causing a constant stream of errors in the log unless
# I do this.
def ignoreTimeout(deferred):
    def handleError(f):
        pass
        #print "ignoring error: ".f.getErrorMessage()
    ...
    def iterativeStore(self, key, value, originalPublisherID=None, age=0):
        ...
        def executeStoreRPCs(nodes):
            ...
            for contact in nodes:
                ignoreTimeout(contact.store(key, value, originalPublisherID, age))
            return nodes
        ...
        def checkResult(result):
            if type(result) == dict:
                ...
                if 'closestNodeNoValue' in result:
                    ...
                    ignoreTimeout(contact.store(key, result[key]))
                ...
                if key in self._dataStore:
                    ...
                    if len(result) > 0:
                        contact = result[0]
                        ignoreTimeout(contact.store(key, value))
