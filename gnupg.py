...
Fixes for Windows GUI and GnuPG 2.1 to make this work with Confidant Mail
were made by Mike Ingle <mike@confidantmail.org>

A unittest harness (test_gnupg.py) has also been added.

Modified by mike@confidantmail.org for use in Confidant Mail
"""
    ...
    def handle_status(self, key, value):
        if key in self.TRUST_LEVELS:
            ...
            self.trust_level = self.TRUST_LEVELS[key]
                     ...
                     "PINENTRY_LAUNCHED","PROGRESS","NEWSIG","EXPORTED",
                     "KEY_CONSIDERED","ENCRYPTION_COMPLIANCE_MODE",
                     "DECRYPTION_COMPLIANCE_MODE","DECRYPTION_KEY",
                     "VERIFICATION_COMPLIANCE_MODE","WARNING","FAILURE"):
            pass
        elif key == "BADSIG":
            self.valid = False
...
class ImportResult(object):
    ...
    def handle_status(self, key, value):
        if key == "IMPORTED" or key == "PINENTRY_LAUNCHED" or key == "KEY_CONSIDERED":
...
class ListKeys(SearchKeys):
    ...
    def fpr(self, args):
		# Check below skips the fingerprints of subkeys. Fix for gpg 2.1.15 - MI
...
class Crypt(Verify, TextHandler):
    ...
    def handle_status(self, key, value):
        if key in ("ENC_TO", "USERID_HINT", "GOODMDC", "END_DECRYPTION",
                   ...
                   "PINENTRY_LAUNCHED","EXPORTED","KEY_CONSIDERED",
                   "ENCRYPTION_COMPLIANCE_MODE","DECRYPTION_COMPLIANCE_MODE",
                   "VERIFICATION_COMPLIANCE_MODE","WARNING","FAILURE"):
...
class GenKey(object):
    ...
    def handle_status(self, key, value):
        if key in ("PROGRESS", "GOOD_PASSPHRASE", "NODATA", "KEY_NOT_CREATED",
                   "PINENTRY_LAUNCHED","EXPORTED","KEY_CONSIDERED","WARNING","FAILURE"):
            pass
...
class Sign(TextHandler):
    ...
    def handle_status(self, key, value):
        if key in ("USERID_HINT", "NEED_PASSPHRASE", "BAD_PASSPHRASE",
                   ...
                   "SC_OP_FAILURE", "SC_OP_SUCCESS","PINENTRY_LAUNCHED",
                   "PROGRESS","EXPORTED","KEY_CONSIDERED",
                   "ENCRYPTION_COMPLIANCE_MODE","DECRYPTION_COMPLIANCE_MODE",
                   "VERIFICATION_COMPLIANCE_MODE","WARNING","FAILURE"):
            pass
...
class GPG(object):
    ...
    def _open_subprocess(self, args, passphrase=False):
        ...
        if sys.platform == 'win32':
            ...
            return Popen(cmd, shell=False, stdin=PIPE, stdout=PIPE, stderr=PIPE, creationflags = CREATE_NO_WINDOW)
        else:
    ...
    def gen_key_input(self, **kwargs):
        ...
        for key, val in list(kwargs.items()):
            ...
            #if str(val).strip():    # skip empty strings # unicode fail
        ...
        for key, val in list(parms.items()):
            out += "%s: %s\n" % (key, val)
        out += "%commit\n"
        if "Subkey-Curve: Ed25519" in out:
            out = out.replace("Subkey-Curve: Ed25519","Subkey-Curve: Curve25519")
            out = out.replace("Key-Length: 2048","Key-Length: 255")
            out = out.replace("Key-Type: ecdsa","Key-Type: eddsa\nKey-Usage: sign\nSubkey-Usage: encrypt")
        return out
